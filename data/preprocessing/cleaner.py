...
class DataCleaner(DataPreprocessorBase):
    ...
    def process(
        ...
        **kwargs,
        ...
        Returns:
            Cleaned dataframe.
        """
        logger.info(f"Cleaning dataset with {len(data)} rows and {len(data.columns)} columns")
                ...
                if categorical_columns:
                    for col in categorical_columns:
                        if col in df.columns and df[col].isna().any():
                            logger.debug(f"Filling missing values in categorical column '{col}'")
        ...
        logger.info(
            f"Cleaning complete: {len(df)} rows, {len(df.columns)} columns"
        )
        return df
    ...
    def _handle_date_issues(self, df: pd.DataFrame) -> None:
        ...
        if duplicates.any():
            ...
            logger.warning(f"Found {n_duplicates} duplicate dates in {self.date_column}")
            
            # If we have other grouping columns, check for duplicates within groups
            group_cols = [col for col in df.columns if col.endswith("_id") or col.endswith("_ID")]
            ...
            if group_cols:
                ...
                if group_duplicates.any():
                    ...
                    numeric_cols = df.select_dtypes(include=["number"]).columns
                    df = df.groupby([self.date_column] + group_cols)[numeric_cols].mean().reset_index()
                    logger.info(f"Aggregated duplicate dates within groups, new shape: {df.shape}")
            else:
            ...
            if group_cols:
                ...
                for group_values, group_df in df.groupby(group_cols):
                    ...
                    group_complete = pd.merge(
                        group_ideal_df, group_df, on=[self.date_column] + group_cols, how="left"
                    )
                ...
                logger.info(f"Created complete date range, new shape: {df_complete.shape}")
