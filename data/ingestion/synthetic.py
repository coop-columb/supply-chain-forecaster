...
class SyntheticDataGenerator(DataIngestionBase):
    ...
    def extract(
        ...
        **kwargs,
        ...
        end_of_month_effect = 1.0 + 0.3 * np.exp(
            -0.5 * ((day_of_month - days_in_month) / 5) ** 2
        )
        ...
        inventory = np.maximum(
            0, np.random.normal(200 - 0.5 * demand, 20, len(demand))
        ).astype(int)
        ...
        orders = np.where(
            inventory < reorder_point, np.maximum(reorder_point * 2 - inventory, 0), 0
        ).astype(int)
