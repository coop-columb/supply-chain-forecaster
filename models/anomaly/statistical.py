...
class StatisticalDetector(ModelBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if method not in ["zscore", "iqr", "mad"]:
            raise ValueError(f"Unknown method: {method}. Must be one of: zscore, iqr, mad")
    ...
    def fit(
        ...
        **kwargs,
        ...
        """
        logger.info(f"Fitting statistical detector {self.name} with method {self.params['method']}")
        ...
        if self.target is None:
            ...
            logger.info(f"No target column specified, using all numeric columns: {self.target_columns}")
        else:
        ...
        **kwargs,
        ...
        for col in self.target_columns:
            ...
            if method == "zscore":
                ...
                scores[series < lower_bound] = (lower_bound - series[series < lower_bound]) / stats["iqr"]
                scores[series > upper_bound] = (series[series > upper_bound] - upper_bound) / stats["iqr"]
        ...
        for col in self.target_columns:
            if col in X.columns and col in self.stats:
                ...
                if method == "zscore":
                    ...
                    lower_bound = stats["q1"] - threshold * stats["iqr"]
                    upper_bound = stats["q3"] + threshold * stats["iqr"]
                    result[col_anomaly] = (series < lower_bound) | (series > upper_bound)
