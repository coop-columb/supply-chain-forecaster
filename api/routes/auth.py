...
class ApiKeyCreate(BaseModel):
    ...
    expires_days: Optional[int] = Field(None, description="Days until key expires (null for no expiration)")
...
class ApiKeyResponse(BaseModel):
    ...
    expires_at: Optional[int] = Field(None, description="Unix timestamp of expiration time")
    scope: Optional[str] = Field(None, description="Permission scope for this key")
...
async def create_api_key(
    ...
    try:
        ...
        if current_user.get("auth_type") != "basic" or "admin" not in current_user.get("roles", []):
            raise HTTPException(
...
async def revoke_api_key(
    ...
    try:
        ...
        if auth_service.revoke_api_key(target_key):
            logger.info(
                f"API key revoked: {key_id} by {current_user['username']}"
            )
