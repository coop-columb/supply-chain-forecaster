...
class ModelService:
    ...
    def __init__(
        ...
        self.training_dir = (
            Path(training_dir) if training_dir else config.TRAINING_MODELS_DIR
        )
        self.deployment_dir = (
            Path(deployment_dir) if deployment_dir else config.DEPLOYMENT_MODELS_DIR
        )
    ...
    def create_model(self, model_type: str, **params) -> ModelBase:
        ...
        try:
            ...
            raise ModelError(f"Error creating model: {str(e)}", model_name=model_type)
    ...
    def load_model(
        ...
        try:
            ...
            raise ModelError(f"Error loading model: {str(e)}", model_name=model_name)

    def save_model(self, model: ModelBase, model_name: Optional[str] = None) -> Dict:
        """
        ...
        try:
            ...
            raise ModelError(f"Error saving model: {str(e)}", model_name=model.name)
    ...
    def deploy_model(self, model_name: str) -> Dict:
        ...
        if not model_files:
            raise ModelError(
                f"Model '{model_name}' not found in {self.training_dir}",
        ...
        # Deploy the model
        try:
            deployed_path = self.model_registry.deploy_model(
                model_file, self.deployment_dir
            )
            ...
            raise ModelError(f"Error deploying model: {str(e)}", model_name=model_name)
    ...
    def delete_model(self, model_name: str, from_deployment: bool = False) -> bool:
        ...
        try:
            ...
            raise ModelError(f"Error deleting model: {str(e)}", model_name=model_name)
