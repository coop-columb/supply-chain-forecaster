...
class AnomalyService:
    ...
    def train_model(
        ...
        **kwargs,
        ...
        try:
            ...
            logger.info(
                f"Successfully trained {model_type} anomaly model '{model_name}'"
            )
            ...
            raise ModelError(
                f"Error training {model_type} anomaly model: {str(e)}",
    ...
    def evaluate_model(
        ...
        **kwargs,
        ...
        """
        logger.info(
            f"Evaluating anomaly model '{model.name}' with {len(X_test)} samples"
        )

        try:
            # Make predictions
            y_pred, scores = model.predict(
                X_test, threshold=threshold, return_scores=True, **kwargs
            )
    ...
    def detect_anomalies(
        ...
        **kwargs,
        ...
        """
        logger.info(
            f"Detecting anomalies with model '{model_name}' in {len(X)} samples"
        )
                ...
                if not anomalies.empty:
                    ...
                    if hasattr(model, "get_feature_importance"):
                        try:
                            ...
                            logger.warning(
                                f"Error getting feature importance: {str(e)}"
                            )
