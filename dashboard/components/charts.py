...
def create_forecast_chart(
    ...
    if len(historical_df) > hist_max_points:
        historical_df = downsample_timeseries(
            historical_df, date_column, value_column, hist_max_points
        )
    ...
    if len(forecast_df) > forecast_max_points:
        ...
        forecast_df = downsample_timeseries(
            forecast_df, date_column, columns_to_include, forecast_max_points
        )
...
def create_anomaly_chart(
    ...
    if len(df) > max_points:
        ...
        if len(normal_df) > normal_max_points:
            columns_to_include = [value_column]
            normal_df = downsample_timeseries(
                normal_df, date_column, columns_to_include, normal_max_points
            )
        ...
        if score_column in anomaly_df.columns:
            ...
            hover_text = [
                ...
                for d, v, s in zip(
                    ...
                    anomaly_df[value_column],
                    anomaly_df[score_column],
                )
            ]
...
def create_feature_importance_chart(
    ...
    if (
        ...
        or feature_col not in importance_df.columns
        or importance_col not in importance_df.columns
    ):
        return html.Div("No data to display")
