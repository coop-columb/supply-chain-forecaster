...
def create_page_layout(title, children=None, fluid=True):
    ...
    return html.Div(
        [
            create_navbar(),
            dbc.Container(
                [html.H1(title, className="mt-4 mb-4"), html.Hr(), *children],
                fluid=fluid,
                className="py-3",
            ),
        ]
...
def create_card(title, children, card_id=None, className=None):
    ...
    """
    return dbc.Card(
        [dbc.CardHeader(html.H4(title)), dbc.CardBody(children)],
        id=card_id,
...
def create_alert(message, color="primary", dismissable=True, is_open=True):
    ...
    """
    return dbc.Alert(
        message, color=color, dismissable=dismissable, is_open=is_open, className="mb-3"
    )
