...
def register_data_exploration_callbacks(app: Dash):
    ...
    def update_time_series(data_json):
        ...
        date_cols = [
            col for col in df.columns if pd.api.types.is_datetime64_dtype(df[col])
        ]
        ...
        for i, col in enumerate(numeric_cols[:3]):
            if x_col:
                charts.append(
                    create_time_series_chart(
                        df, x_col, [col], f"{col} Over Time", f"exploration-{i}"
                    )
                )
            else:
                # Use index as x-axis
        ...
        return html.Div(
            [
                ...
                dbc.Row(
                    [dbc.Col(chart, width=12, className="mb-4") for chart in charts]
                ),
            ]
        )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return html.Div(
                    [
                        dbc.Alert(
                            f"Model '{result['model_name']}' trained successfully!",
                            color="success",
                        ),
                        dbc.Card(
                            [
                ...
                return dbc.Alert(
                    f"Error training model: {response.text}", color="danger"
                )
...
def register_model_management_callbacks(app: Dash):
    ...
    @app.callback(
        ...
        [Input("refresh-trained-models-button", "n_clicks"), Input("url", "pathname")],
        [State("api-url", "data")],
    )
    def refresh_trained_models(n_clicks, pathname, api_url):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return html.Div(
                    f"Error fetching trained models: {response.status_code}"
                )
        except Exception as e:
            return html.Div(f"Error connecting to API: {str(e)}")
    ...
    @app.callback(
        ...
        [Input("refresh-deployed-models-button", "n_clicks"), Input("url", "pathname")],
        [State("api-url", "data")],
    )
    def refresh_deployed_models(n_clicks, pathname, api_url):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return html.Div(
                    f"Error fetching deployed models: {response.status_code}"
                )
        except Exception as e:
            return html.Div(f"Error connecting to API: {str(e)}")
    ...
    @app.callback(
        ...
        [State("trained-models-table", "data"), State("api-url", "data")],
    )
    def show_trained_model_details(selected_rows, data, api_url):
        ...
        try:
            response = requests.get(
                f"{api_url}/models/{model_id}?model_type={model_type}"
            )
            if response.status_code == 200:
    ...
    @app.callback(
        ...
        [State("deployed-models-table", "data"), State("api-url", "data")],
    )
    def show_deployed_model_details(selected_rows, data, api_url):
        ...
        try:
            response = requests.get(
                f"{api_url}/models/{model_id}?model_type={model_type}&from_deployment=true"
            )
            if response.status_code == 200:
                model_info = response.json()
    ...
    @app.callback(
        ...
        [State("trained-model-selected-model", "data"), State("api-url", "data")],
    )
    def deploy_model(n_clicks, model_id, api_url):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return dbc.Alert(
                    f"Error deploying model: {response.text}", color="danger"
                )
        except Exception as e:
