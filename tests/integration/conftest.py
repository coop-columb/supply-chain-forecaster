...
def sample_anomaly_data():
    ...
    df = pd.DataFrame(
        {
            ...
            "demand": 100
            + 20 * np.sin(np.arange(n) * (2 * np.pi / 30))
            + np.random.normal(0, 5, n),
            "inventory": 200
            - 15 * np.sin(np.arange(n) * (2 * np.pi / 30))
            + np.random.normal(0, 8, n),
            "lead_time": 5
            + 2 * np.sin(np.arange(n) * (2 * np.pi / 90))
    ...
    df.loc[anomaly_indices, "stockout_risk"] += np.random.normal(
        0.4, 0.1, len(anomaly_indices)
    )
