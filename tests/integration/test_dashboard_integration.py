...
def test_dashboard_pages(dashboard_app):
    ...
    for page in expected_pages:
        assert (
            page in dashboard_app.pages_registry
        ), f"Expected page '{page}' not found in dashboard"


def test_data_upload_component(
    dashboard_app, sample_time_series_data, integration_test_env
):
...
def test_dashboard_model_selection_component(dashboard_app):
    ...
    try:
        ...
        assert (
            forecasting_component is not None
        ), "Forecasting model selection creation failed"
    ...
    try:
        ...
        try:
            from dashboard.pages.data_exploration import (
                layout as data_exploration_layout,
            )

            assert (
                data_exploration_layout is not None
            ), "Data exploration page layout is None"
        except ImportError as e:
        ...
        try:
            from dashboard.pages.model_management import (
                layout as model_management_layout,
            )

            assert (
                model_management_layout is not None
            ), "Model management page layout is None"
        except ImportError as e:
