...
def test_forecasting_endpoints(api_client, sample_time_series_data, integration_test_env):
...
def test_anomaly_detection_endpoints(api_client, sample_anomaly_data, integration_test_env):
    ...
    try:
        ...
        if "is_anomaly_true" in df_result.columns and "is_anomaly_pred" in df_result.columns:
            accuracy = (df_result["is_anomaly_true"] == df_result["is_anomaly_pred"]).mean()
