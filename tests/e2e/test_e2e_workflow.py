    ...
    df = pd.DataFrame(
        {
            ...
            "demand": 100
            + np.linspace(0, 20, n)
            + 10 * np.sin(np.arange(n) * (2 * np.pi / 7))
            + np.random.normal(0, 5, n),
            "price": 10
            + np.sin(np.arange(n) * (2 * np.pi / 180))
...
def test_full_forecasting_workflow(e2e_environment):
    ...
    try:
        ...
        response = requests.post(
            "http://localhost:8000/forecasting/train", json=train_data
        )
    ...
    try:
        ...
        response = requests.post(
            "http://localhost:8000/forecasting/predict", json=forecast_data
        )
...
def test_docker_deployment():
    ...
    try:
        ...
        try:
            ...
            ), f"API health check failed: {response.text}"
            assert (
                response.json().get("status") == "ok"
            ), "API not reporting healthy status"
        except requests.exceptions.RequestException as e:
