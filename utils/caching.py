...
def memoize_with_expiry(ttl: Optional[datetime.timedelta] = None):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            if not hasattr(config, "ENABLE_RESPONSE_CACHING") or not config.ENABLE_RESPONSE_CACHING:
            ...
            expired_keys = [
                k for k, (_, ts) in _prediction_cache.items() 
                if now - ts > ttl
            ]
...
def get_prediction_cache_stats() -> Dict[str, int]:
    ...
    active_entries = sum(1 for _, ts in _prediction_cache.values() if now - ts < _DEFAULT_CACHE_TTL)
...
class ModelCache:
    ...
    def get(self, model_id: str) -> Optional[Any]:
        ...
        """
        if not hasattr(config, "ENABLE_MODEL_CACHING") or not config.ENABLE_MODEL_CACHING:
            return None
