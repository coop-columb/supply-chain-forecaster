...
cpu_usage_percent = Gauge("cpu_usage_percent", "CPU usage in percent")
    ...
    async def wrapper(*args, **kwargs):
        ...
        if PROFILING_ENABLED and config.ENABLE_PROFILING:
            ...
            try:
                ...
                with (
                    profile_time(endpoint_name, "api"),
                    profile_memory(endpoint_name, "api"),
                ):
...
def track_model_prediction(func: Callable) -> Callable:
    ...
    def wrapper(*args, **kwargs):
        ...
        if PROFILING_ENABLED and config.ENABLE_PROFILING:
            ...
            try:
                with (
                    profile_time(operation_name, "model"),
                    profile_memory(operation_name, "model"),
                ):
                    start_time = time.time()
                ...
                prediction_duration_seconds.labels(model_name=model_name).observe(
                    duration
                )
...
class PrometheusMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        ...
        if (
            PROFILING_ENABLED
            and hasattr(config, "ENABLE_PROFILING")
            and config.ENABLE_PROFILING
        ):
