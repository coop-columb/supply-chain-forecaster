...
def handle_outliers(
    ...
    if strategy == "remove":
        ...
        if method == "zscore":
            ...
            result_df.loc[outliers, column] = result_df[column].mean() + np.sign(z_scores[outliers]) * threshold * result_df[column].std()
        ...
        if not isinstance(result_df.index, pd.DatetimeIndex):
            ...
            if result_df[column].isna().any():
                result_df[column] = result_df[column].fillna(method='ffill').fillna(method='bfill')
...
def impute_missing_values(
    ...
    if method == "linear":
        ...
        if not isinstance(result_df.index, pd.DatetimeIndex):
            logger.warning("Using time method for non-datetime index, falling back to linear")
            result_df[column] = result_df[column].interpolate(method="linear")
        else:
...
def create_time_features(df: pd.DataFrame, date_column: str = None) -> pd.DataFrame:
    ...
    if date_column is not None:
        ...
        if not isinstance(result_df.index, pd.DatetimeIndex):
            raise ValueError("Dataframe index is not a DatetimeIndex and no date column provided")
...
def create_lag_features(
    ...
    if group_by is not None:
        for lag in lags:
            result_df[f"{column}_lag_{lag}"] = result_df.groupby(group_by)[column].shift(lag)
    else:
        for lag in lags:
...
def create_rolling_features(
    ...
    """
    logger.info(f"Creating rolling features for column '{column}' with windows {windows}")
    ...
    for window in windows:
        if group_by is not None:
            ...
            for func in functions:
                if func == "mean":
                    result_df[f"{column}_rolling_{window}_{func}"] = result_df[column].rolling(
...
def add_holiday_features(df: pd.DataFrame, country: str = "US") -> pd.DataFrame:
    ...
    try:
        ...
        logger.warning("pandas.tseries.holiday not available, skipping holiday features")
        return df
    ...
    if not isinstance(result_df.index, pd.DatetimeIndex):
        logger.warning("Dataframe index is not a DatetimeIndex, skipping holiday features")
        return result_df
    ...
    if country == "US":
        ...
        result_df["is_day_before_holiday"] = result_df.index.isin(holidays - pd.Timedelta(days=1)).astype(int)
        result_df["is_day_after_holiday"] = result_df.index.isin(holidays + pd.Timedelta(days=1)).astype(int)
        
        # Add business day features
        bday = CustomBusinessDay(calendar=cal)
        result_df["is_business_day"] = result_df.index.map(lambda x: x.dayofweek < 5 and x not in holidays).astype(int)
        
        # Add specific US holidays
        for holiday_date, holiday_name in zip(holidays, cal.holiday_names):
            mask = (result_df.index == holiday_date)
            if mask.any():
                holiday_col_name = f"is_{holiday_name.lower().replace(' ', '_').replace('-', '_')}"
                result_df[holiday_col_name] = 0
                result_df.loc[mask, holiday_col_name] = 1
    else:
        logger.warning(f"Holiday features for country '{country}' not implemented, using US instead")
