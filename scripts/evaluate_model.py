...
def load_benchmarks(benchmark_file):
    ...
    if not os.path.exists(benchmark_file):
        logger.warning(
            f"Benchmark file {benchmark_file} not found. Using default benchmarks."
        )
        return {
            "forecasting": {
...
def evaluate_forecasting_model(model_path, benchmarks):
    ...
    benchmark_metrics = benchmarks["forecasting"].get(
        model_type, {"mape": 20.0, "rmse": 30.0}
    )

    passed = (
        metadata["metrics"]["mape"] <= benchmark_metrics["mape"]
        and metadata["metrics"]["rmse"] <= benchmark_metrics["rmse"]
    )
...
def evaluate_anomaly_model(model_path, benchmarks):
    ...
    # Compare with benchmarks
    benchmark_metrics = benchmarks["anomaly"].get(
        model_type, {"f1_score": 0.75, "precision": 0.70, "recall": 0.75}
    )

    passed = (
        metadata["metrics"]["f1_score"] >= benchmark_metrics["f1_score"]
        and metadata["metrics"]["precision"] >= benchmark_metrics["precision"]
        and metadata["metrics"]["recall"] >= benchmark_metrics["recall"]
    )
...
def main():
    ...
    if not model_files:
        ...
        with open(args.output_file, "w") as f:
            json.dump(
                {"evaluated_models": [], "passed_models": [], "failed_models": []},
                f,
                indent=2,
            )
        return 0
