...
from dashboard.components.charts import create_forecast_chart, create_time_series_chart
...
def generate_test_data(rows=10000):
    ...
    df = pd.DataFrame({"demand": demand, "store_id": 1, "product_id": 1, "date": dates})
...
def test_data_downsampling(df, repeats=5):
    ...
    for _ in range(repeats):
        ...
        for col in value_cols:
            fig.add_trace(
                go.Scatter(
                    x=downsampled_df[date_col], y=downsampled_df[col], mode="lines"
                )
            )
    ...
    return {
        ...
        "downsampled_data_points": len(
            downsample_timeseries(df, date_col, value_cols, max_points=500)
        ),
        "without_downsampling_ms": no_downsample_time,
        "with_downsampling_ms": downsample_time,
        "speedup_factor": (
            no_downsample_time / downsample_time if downsample_time > 0 else 0
        ),
    }
...
def test_chart_optimization(df, repeats=5):
    ...
    return {
        ...
        "speedup_factor": (
            no_optimization_time / optimization_time if optimization_time > 0 else 0
        ),
    }
...
def test_component_caching(df, repeats=3):
    ...
    def cached_component_fn(test_df):
        ...
        return create_time_series_chart(
            test_df, date_col, value_cols, "Test Chart", "test"
        )
    ...
    return {
        ...
        "speedup_factor": (
            cache_miss_time / avg_cache_hit_time if avg_cache_hit_time > 0 else 0
        ),
    }
...
def run_all_tests():
    ...
    logger.info(
        f"Generated test data: small ({len(small_df)} rows), large ({len(large_df)} rows)"
    )
    ...
    logger.info(
        f"Data downsampling speedup: {downsampling_results['speedup_factor']:.2f}x"
    )
...
def plot_results(results):
    ...
    if "data_downsampling" in results:
        ...
        plt.bar(
            ["Without Downsampling", "With Downsampling"],
            [
                downsampling["without_downsampling_ms"],
                downsampling["with_downsampling_ms"],
            ],
        )
        plt.title(
            f"Data Downsampling Performance\n(Speedup: {downsampling['speedup_factor']:.2f}x)"
        )
        plt.ylabel("Time (ms)")
    ...
    with open(report_file, "w") as f:
        ...
        if "data_downsampling" in results:
            ...
            f.write(
                f"- Data reduction: {100 * (1 - down['downsampled_data_points'] / down['original_data_points']):.1f}%\n"
            )
            f.write(
                f"- Without downsampling: {down['without_downsampling_ms']:.2f}ms\n"
            )
        ...
        if "chart_optimization" in results:
            ...
            f.write(
                f"- Without optimization: {chart['without_optimization_ms']:.2f}ms\n"
            )
        ...
        if "component_caching" in results:
            ...
            f.write(
                f"- Average cache hit time: {cache['avg_cache_hit_time_ms']:.2f}ms\n"
            )
        ...
        if len(speedups) == 4:  # If we calculated the combined speedup
            ...
            f.write(
                "- Note: This is an estimate based on the multiplicative effect of all optimizations.\n"
            )
            f.write(
                "  Actual performance may vary depending on specific use cases and data characteristics.\n\n"
            )

        # Add conclusions and recommendations
        f.write("## Conclusions\n")
        f.write(
            "The dashboard optimizations provide significant performance improvements:\n\n"
        )
        f.write(
            "1. Data downsampling is most effective for large datasets with thousands of points\n"
        )
        f.write(
            "2. Component caching provides the greatest benefit for complex components that are frequently reused\n"
        )
        f.write(
            "3. Chart optimization helps reduce client-side rendering time and improves interactivity\n\n"
        )
        f.write(
            "These optimizations are particularly important for production deployments with many concurrent users\n"
        )
        f.write(
            "and/or large datasets. The performance benefits scale with the size and complexity of the data.\n"
        )
