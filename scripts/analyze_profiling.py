...
def create_dashboard_performance_report(data: Dict, output_dir: str):
    ...
    if dashboard_data:
        ...
        for component, metrics in dashboard_data.items():
            report.append(f"| {component} | {metrics['mean_ms']:.2f} | {metrics['median_ms']:.2f} | "
...
def create_profiling_overview(data: Dict, output_dir: str):
    ...
    if "api" in data and data["api"]:
        html.append("<table>")
        html.append("<tr><th>Endpoint</th><th>Count</th><th>Mean (ms)</th><th>Median (ms)</th><th>P95 (ms)</th><th>Max (ms)</th><th>Status</th></tr>")
        ...
        html.append(f"<img src='api_endpoint_comparison.png' alt='API Endpoint Comparison' style='width: 100%;'>")
        html.append("</div>")
    else:
    ...
    if "model" in data and data["model"]:
        ...
        training_data = {k: v for k, v in data["model"].items() if k.startswith("train_")}
        
        if training_data:
            html.append("<table>")
            html.append("<tr><th>Model Type</th><th>Mean Time (s)</th><th>Max Time (s)</th></tr>")
            ...
            html.append(f"<img src='model_training_comparison.png' alt='Model Training Comparison' style='width: 100%;'>")
            html.append("</div>")
        else:
        ...
        inference_data = {k: v for k, v in data["model"].items() if k.startswith("infer_")}
        
        if inference_data:
            html.append("<table>")
            html.append("<tr><th>Model Type</th><th>Data Size</th><th>Mean Time (ms)</th><th>P95 Time (ms)</th></tr>")
            ...
            html.append(f"<img src='model_inference_comparison.png' alt='Model Inference Comparison' style='width: 100%;'>")
            html.append("</div>")
        else:
    ...
    if "dashboard" in data and data["dashboard"]:
        html.append("<table>")
        html.append("<tr><th>Component</th><th>Mean (ms)</th><th>Median (ms)</th><th>P95 (ms)</th><th>Max (ms)</th></tr>")
    ...
    # API recommendations
    if "api" in data and data["api"]:
        slow_endpoints = [endpoint for endpoint, metrics in data["api"].items() 
                        if metrics["mean_ms"] > 100]
        if slow_endpoints:
            html.append(f"<li class='warning'>Slow API endpoints detected: {', '.join(slow_endpoints)}. Consider optimizing these endpoints.</li>")
        else:
            html.append("<li class='good'>All API endpoints are responding within acceptable time limits.</li>")
    ...
    if "model" in data and data["model"]:
        ...
        if training_data:
            ...
            html.append(f"<li>Slowest model to train: <strong>{model_type}</strong> ({train_time:.2f} seconds). Consider optimizing or pre-training this model.</li>")
        
        # Find slowest inference model
        if inference_data:
            large_inference = {k: v for k, v in inference_data.items() if k.endswith("_large")}
            if large_inference:
                slowest_inference = max(large_inference.items(), key=lambda x: x[1]["mean_ms"])
                model_type = slowest_inference[0].replace("infer_", "").replace("_large", "")
                infer_time = slowest_inference[1]["mean_ms"]
                
                html.append(f"<li>Slowest model for large dataset inference: <strong>{model_type}</strong> ({infer_time:.2f} ms). Consider optimizing or implementing batching.</li>")
    
    # General recommendations
    html.append("<li>Consider implementing model caching to reduce model loading overhead.</li>")
    html.append("<li>For large datasets, implement chunked processing and streaming responses.</li>")
    html.append("<li>Optimize dashboard loading by implementing lazy loading of components.</li>")
    html.append("<li>Consider adding data caching for frequently accessed datasets.</li>")
...
def main():
    parser = argparse.ArgumentParser(description="Analyze and visualize profiling results")
    parser.add_argument("--input", type=str, default="profiling_results.json", 
                      help="Input file with profiling results")
    parser.add_argument("--output-dir", type=str, default="profiling_visualizations", 
