#!/bin/bash
# Script to set up an Azure Kubernetes Service (AKS) cluster and configure it for the Supply Chain Forecaster CD pipeline
...
RESOURCE_GROUP="supply-chain-forecaster-rg"
LOCATION="eastus"
NODE_COUNT=2
VM_SIZE="Standard_DS2_v2"

echo "=== Supply Chain Forecaster: Azure Kubernetes Setup ==="
echo ""
echo "This script will set up an AKS cluster and configure it for use with the CD pipeline."
echo "It will create the following resources in Azure:"
...
if ! command -v kubectl &> /dev/null; then
    echo "kubectl not found. Would you like to install it using Azure CLI? (y/n)"
...
# Get the kubeconfig
az aks get-credentials --resource-group ${RESOURCE_GROUP} --name ${CLUSTER_NAME} --file .github_environment_setup/kube_config.yaml
...
echo "===== Setup Complete ====="
echo ""
echo "Your AKS cluster is now ready to use with the Supply Chain Forecaster CD pipeline."
echo ""
...
echo "This cluster will incur charges on your Azure account. To delete it when not needed:"
