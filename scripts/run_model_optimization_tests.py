...
from utils.profiling import get_profiling_stats, profile_time, reset_profiling_stats
...
def test_model_caching(model_class, data, repetitions=5):
    ...
    return {
        ...
        "speedup_factor": first_prediction_time
...
def plot_results(results):
    ...
    if "model_caching" in results:
        ...
        plt.bar(
            ...
            [
                caching_results["cache_miss_time_ms"],
                caching_results["average_cache_hit_time_ms"],
            ],
        )
        plt.title(
            f"Model Caching Performance (Speedup: {caching_results['speedup_factor']:.2f}x)"
        )
        plt.ylabel("Time (ms)")
    ...
    with open(report_file, "w") as f:
        ...
        if "model_caching" in results:
            ...
            f.write(
                f"- Cache miss time: {caching_results['cache_miss_time_ms']:.2f}ms\n"
            )
            f.write(
                f"- Average cache hit time: {caching_results['average_cache_hit_time_ms']:.2f}ms\n"
            )
