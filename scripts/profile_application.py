...
from utils.profiling import get_profiling_stats, profile_cpu, profile_memory, profile_time, reset_profiling_stats
...
# Sample data paths
SAMPLE_DATA_PATH = os.path.join(os.path.dirname(__file__), "..", "examples", "sample_data")
...
def profile_api_endpoints(base_url: str, iterations: int = 10, auth: Optional[Dict] = None):
    """
...
def profile_model_training(api_url: str, model_types: List[str] = None):
    ...
    for model_type in model_types:
        ...
        feature_columns = ["date", "temperature", "promotion"] if "date" in df.columns else df.columns[:-1].tolist()
        ...
        # Profile training
        with profile_time(f"train_{model_type}", "model"), profile_memory(f"train_{model_type}", "model"):
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(f"Training failed: {response.status_code} - {response.text}")
...
def profile_model_inference(api_url: str, model_type: str, model_name: str, df: pd.DataFrame):
    """
    ...
    for size_name, size_df in data_sizes:
        ...
        with profile_time(f"infer_{model_type}_{size_name}", "model"), profile_memory(f"infer_{model_type}_{size_name}", "model"):
            try:
                response = requests.post(
                ...
                if response.status_code == 200:
                    result = response.json()
                    logger.info(f"Inference successful for {size_name} data ({len(size_df)} rows)")
                else:
                    logger.warning(f"Inference failed: {response.status_code} - {response.text}")
...
def profile_dashboard_loading(dashboard_url: str):
    ...
    logger.info("Note: For detailed dashboard profiling, use browser developer tools or Selenium")
...
def main():
    ...
    parser.add_argument("--api-url", type=str, default=f"http://{config.API_HOST}:{config.API_PORT}", 
                      help="API URL")
    parser.add_argument("--dashboard-url", type=str, default=f"http://{config.DASHBOARD_HOST}:{config.DASHBOARD_PORT}", 
                       help="Dashboard URL")
    ...
    parser.add_argument("--api-only", action="store_true", 
                      help="Profile only API endpoints")
    parser.add_argument("--model-only", action="store_true", 
                       help="Profile only model training and inference")
    parser.add_argument("--dashboard-only", action="store_true", 
                        help="Profile only dashboard loading")
