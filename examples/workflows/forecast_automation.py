...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("forecast_automation.log")],
)
...
def retrain_models(client, data_path, model_configs):
    ...
    for config in model_configs:
        try:
            ...
            logger.exception(
                f"Exception during training of {config.get('base_name', 'unknown')}: {str(e)}"
            )
...
def generate_forecasts(client, data_path, forecast_configs):
    ...
    for config in forecast_configs:
        try:
            ...
            if result["status"] == "success":
                logger.info(
                    f"Successfully generated forecast with model: {config['model_name']}"
                )
                ...
                logger.error(
                    f"Failed to generate forecast with model: {config['model_name']}"
                )
                logger.error(f"Error: {result.get('message', 'Unknown error')}")

        except Exception as e:
            logger.exception(
                f"Exception during forecast with {config.get('model_name', 'unknown')}: {str(e)}"
            )
...
def main():
    ...
    parser.add_argument(
        "--mode",
        choices=["schedule", "retrain", "forecast"],
        default="schedule",
    ...
    parser.add_argument(
        "--api-key", default=None, help="API key for authentication (optional)"
    )
